...
import requests
from django.shortcuts import render, redirect, get_object_or_404,HttpResponse         # get_object_or_404 - used to checks whether the
...
def show(request):
    ...
    return render(request, 'show.html', {'allappointments': allappointments})   # This is the code for fetching data from database


def delete(request, id):          # ID checks for the id value assigned to each record in the database.
    myappoint =myAppointment.objects.get(id = id)
    myappoint.delete()
    return redirect('show')     # Ensures we remain in the same page after the 'delete' action.
...
def edit(request, id):
    ...
    if request.method == 'POST':
        editappointment.name = request.POST.get('name')        # The values in brackets are names of name attributes
